...
namespace Presentation
{
    public partial class ExamDialog : Form
    {
        ...
        private DateTimePicker _dtpExamDate;
        ...
        private void InitUI()
        {
            ...
            var l2 = Lbl("Group *", 24, 126);
            _cmbGroup = new StyledComboBox { Width = 360, Location = new Point(24, 146) };

            var l3 = Lbl("Full Mark *", 24, 192);
            _txtFullMark = new StyledTextBox { Width = 360, Height = AppTheme.InputHeight, Location = new Point(24, 212) };

            var l4 = Lbl("Exam Date *", 24, 258);
            _dtpExamDate = new DateTimePicker
            {
                Width = 360,
            ...
            _lblError = new Label { Font = AppTheme.FontSmall, ForeColor = AppTheme.Danger, BackColor = Color.Transparent, AutoSize = false, Width = 360, Height = 18, Location = new Point(24, 326) };

            _btnSave = new RoundedButton { Text = "Save", Width = 140, Height = AppTheme.ButtonHeight, Location = new Point(24, 350) };
            _btnSave.Click += async (s, e) => await SaveAsync();

            _btnCancel = new GhostButton { Text = "Cancel", Width = 100, Height = AppTheme.ButtonHeight, Location = new Point(176, 350) };
            _btnCancel.Click += (s, e) => { DialogResult = DialogResult.Cancel; Close(); };

            Controls.AddRange(new Control[] { lblTitle, l1, _txtName, l2, _cmbGroup, l3, _txtFullMark, l4, _dtpExamDate, _lblError, _btnSave, _btnCancel });
        ...
        private async Task SaveAsync()
        {
            ...
            { _lblError.Text = "Exam name required."; return; }

            if (_cmbGroup.SelectedItem is not Group g)
            { _lblError.Text = "Select a group."; return; }

            if (!int.TryParse(_txtFullMark.Text, out int fm) || fm <= 0)
            { _lblError.Text = "Valid full mark required."; return; }

            _btnSave.Enabled = false;
            var r = await _examService.CreateAsync(g.Id, _txtName.Text.Trim(), fm, _dtpExamDate.Value);
